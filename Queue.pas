...
var
  r: PNode;
begin
  if p = nil then                 //если список пуст, выходим
    Exit;

  if (p.inf2 = k) and (p^.Next <> nil) then      // первый элемент искомый, далее список
  begin
    r:= p;
    r^.Pred^.Next:= nil;
    p:= p^.Pred;
  ...
  if (p.Pred = nil) and (p.inf2 = k) then       // первый элемент искомый, далее список
  begin
   p:= nil;
   Exit;
  end;

  if p.inf2 <> k then            // первый элемент не искомый
  begin
    r:= p;
    while r.inf2 <> k do
      r:= r^.Next;

    if r.Next = nil then      // искомый элемент - последний                
    begin
      r^.Pred^.Next:= nil;
      Dispose(r);
    end
